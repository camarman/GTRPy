...
def tensor_producer(diag_comp, coord_sys, xtensor, xtensor_type=''):
    """
    ...
    Args:
        ...
        xtensor [string]: The name of the desired tensor given as a string
        xtensor_type [string]: The type of the tensor. Given in terms of 'u': contravariant
        and 'd': covariant
    ...
    if xtensor == 'Metric Tensor':
        mt = MetricTensor(diag_comp, coord_sys)
        metric_tensor = mt.get_metrictensor()
        if xtensor_type == '':   # the default type of the given tensor
    ...
    if xtensor == 'Kretschmann Scalar':
        ks = KretschmannScalar(diag_comp, coord_sys)
        kret_scalar = ks.get_kretschmannscalar()
        return '$$K = ' + latex(kret_scalar) + '$$'
                 

def tensor_component_producer(diag_comp, coord_sys, xtensor, component='', xtensor_type=''):
    """
    The function that will obtain inputs from the GUI in the GRTC_GUI.py file to produce
    ...
    Args:
        ...
        diag_comp [list]: Diagonal components of the metric tensor
        xtensor [string]: The name of the desired tensor given as a string
        component [string]: The desired component of the tensor
        xtensor_type [string]: The type of the tensor. Given in terms of 'u': contravariant
        and 'd': covariant
