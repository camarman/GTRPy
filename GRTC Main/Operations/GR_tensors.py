################### GRTC - GENERAL RELATIVITY - TENSORS ###################
...
def Simplify(xtensor):
    ...
    """
    try:
        return simplify(nsimplify(xtensor)) # if all components are 0 the nsimplify gives error
    except:
        return xtensor
...
class MetricTensor(object):
    def __init__(self, diag_comp, coord_sys):
        ...
        self.metric_type = 'dd'
        self.metric_obj = Array(diag(self.diag_comp))
        self.inverse_metric_obj = Array(diag(1/self.diag_comp))
